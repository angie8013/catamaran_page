...
function reorganizeContentForExpanded(targetContent) {
  ...
  if (additionalInfo && detailsGrid) {
    ...
    // ✅ Si es responsive, insertar additional-info después de "no incluye"
    if (window.innerWidth < 768 && notIncludesSection && additionalInfo) {
      notIncludesSection.parentNode.insertBefore(additionalInfo, notIncludesSection.nextSibling);
    } else {
      // En escritorio, mantenerlo en la columna izquierda
      const leftColumn = targetContent.closest(".experience-card").querySelector(".left-column");
      if (leftColumn && !leftColumn.contains(additionalInfo)) {
        leftColumn.appendChild(additionalInfo);
      }
    }
...
window.resetChatSession = function resetChatSession() {
  ...
  // Reorganizar si se redimensiona la ventana en modo responsive
  window.addEventListener("resize", () => {
    document.querySelectorAll(".experience-details.expanded").forEach(section => {
      reorganizeContentForExpanded(section);
    });
  });
